...
```{r} set.seed(579) sample(x = c(-1, 1), size = 1)}
```
...
```{r} set.seed(33) bin_add <- replicate(1000,                       sum(                        sample(x = c(-1, 1), size = 10, replace = TRUE))) hist(bin_add)}
```
...
```{r} set.seed(444) growth <- replicate(1000,                      prod(1 + runif(10, min = 0, max = 0.1))) hist(growth)}
```
