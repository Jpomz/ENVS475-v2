...
-   One-way ANOVA is like a t-test with 3 or more groups instead of 2
...
$$E[y_j] = \beta_0 + \beta_1x_{1j} + \beta_2x_{2j}$$

-   subscript $j$ indicates which group we are estimating

-   This is where the "dummy" variables become more important to understand

-   The following table shows the values of $x_{1j}$ and $x_{2j}$ for each group
...
-   For the `ctrl` group, both $\beta_1$ and $\beta_2$ are cancelled out because they are multiplied by 0

    -   so $\beta_0$ is the estimated mean of our `ctrl` group

-   For the `trt1` group, $\beta_1$ stays (because it is multiplied by 1) but $\beta_2$ is cancelled out

    -   So $\beta_1$ is the *difference* in means between the `ctrl` and the `trt1` groups

-   For the `trt1` group, $\beta_2$ stays (because it is multiplied by 1) but $\beta_1$ is cancelled out

    -   So $\beta_2$ is the *difference* in means between the `ctrl` and the `trt2` groups

-   We will return to this with the `model.matrix()` function shortly

### Check pre-analysis assumptions

-   Before we fit our linear model, we need to make sure we pass our assumptions:

    1.  Equal Variance within between groups (boxplot of data)

    2.  Normality of residuals (Q-Q plot of data)

    3.  Equal Variance of Residuals (fitted vs residuals of the *model*)
...
-   Note that there may be a slight pattern of declining variance from left to right
    -   This is OK here, but you want to look out for the "megaphone" shape
-   
...
-   Let's revisit the "dummy" variables

-   We can see the value of the dummy variables for each group

-   First recall that the `PlantGrowth` data has 30 rows, 10 for each group

```{r}
PlantGrowth
```

-   Now let's look at the dummy variables

```{r}
model.matrix(plant_lm)
```

-   The value for $x_1$ is 0 (2nd column) for the first and last 10 rows (`ctrl` and `trt2` groups) but is 1 for the middle 10 rows (group `trt1`)

-   Likewise, The value for $x_2$ is 0 (3rd column) for the first 20 rows (`ctrl` and `trt1` groups) but is 1 for the last 10 rows (group `trt2`)

-   Note that the value in the first column is 1 for everything.

    -   Technically, there is a third dummy variable, $x_0$

    -   It is 1 for all groups, which just re-emphasizes the fact that our beta coefficients are showing us the *difference* between our reference and "other" groups.
...
-   Note that the residual\_{df}, F-values, and p-values all changed slightly with the addition of a blocking factor.
