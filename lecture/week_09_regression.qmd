...
-   Let's return to data which does not pass assumptions

-   For this, we sill use the `msleep` data which is in the `ggplot2` pacakage
...
-   This data has information on mammal body size, and their sleep patterns

-   We will be looking at `bodwt` (X or predictor variable) to predict `sleep_total` (Y, or response variable)
...
```
```{r}
msleep_lm <- lm(brainwt ~ bodywt, data = msleep)
```
...
-   We can do a log~10~ transformation on this data to see how it looks  

-   For plotting, we have two options for log~10~ transformation
...
-   notice the scales on the axes

#### Log~10~ Transform the variables
...
-   Notice the name of the axes  

#### Fit the model on tranformed variables  

```{r}
msleep_log_lm <- lm(log(brainwt) ~ log(bodywt), data = msleep)
```
...
```{r}
ggplot(msleep_log_lm,
       aes(x = .fitted, y = .resid)) +
    geom_point() +
    geom_hline(yintercept = 0) +
  theme_bw()
```
-   Continue with interpreting the log-log transformed model  

-   Note that the line for this model has both x and y on the log-scale

-   We need to transform the estimated values to "real" numbers on the original scale  

```{r}
summary(msleep_log_lm)
```

$$
Log( \hat{\text{brainwt}}) = -4.8 + 0.76 * Log(\text{bodywt})
$$

-   What is the estimated brain wt for a 50 kg mammal?
...
-   Transformations can deal with data that fails assumptions  

-   There are many transformations out there  

-   Always back-calculate your results to the original scale  
