...
Split the data based on species, and make two new data frames, one called `fortis` and one called `scandens`.\
...
Print the `scandens` object to your console Also, add the following code to your answer and ensure the output is either `character` or `factor`
...
1.3\
Fit an intercept-only model using the `lm()` function, and save it in an object called `flm0` (short for "finch linear model 0").
...
1.4\
Use the `display()` function to show the results of `flm0`.\
...
1.6\
Calculate and print the global mean of `blength` from the `scandens` object.
...
1.7\
Calculate and print the sd for `blength`. from the `scandens` object.
...
Use ggplot to make a scatter plot of the `blength` observations separated by year.
...
2.3\
Use `lm()` to fit a model which includes `year` as a predictor variable. Save this model output in an object called `flm1`, and use `display()` to print out the results in your console.
...
2.4\
Based on the information from 2.3 above, enter a simple aritmetic equation which estimates the the mean beak length for the *scandens* species in *1975*?
...
2.5 Make a scatter plot of the `scandes` data which separates the observations by year, and includes a point for the estimated group means.\
...
3.2\
Use `coefplot()`and the `flm1` object to display a graphical representation of the approximate 95% CI for the estimated difference in beak length between 1975 and 2012.
...
# problem 5 relevel the year factor
