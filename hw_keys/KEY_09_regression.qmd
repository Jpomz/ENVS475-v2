...
```{r}
island_lm <- lm(log10_Nspp ~ log10_area, data = island)
```
...
# the log10_area variable is significant (DF_1_15 = 21.118, p < 0.001). This means that the response variable does change with island area. 
```
...
```{r}
# Intercept
# the mean estimate and SE for the intercept coefficient is 1.32 (0.14). We reject the null hypothesis for the intercept, and conclude that it is not equal to 0 (t = 9.286, p < 0.001). 
...
```{r}
# log10_nspp = 1.32 + 0.33 * log10_area
```
